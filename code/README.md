...
npm --prefix code run inspect:horizon-sensitivity -- --dataset ../dataset
~~~
...
The default `inclusive_90_dates_v1` policy covers request-date offsets **0 through 89 inclusive: exactly 90 dates**. The end is request date plus 89 days, following the literal “next 90 days” requirement. Configuration `horizonDays` denotes the final inclusive offset, not the date count. Its deterministic hash is reported separately from the recurrence policy hash. No clock is consulted. Historical cash is never replayed against the profile snapshot. Supplied future cash identities are retained once. Supported active recurrence uses only the selected strict safety estimate; its reference median never enters feasibility calculations. Unsupported, stale or ambiguous income is excluded. Uncertain expenses retain provenance and structured unresolved obligations; missing debit amounts block capacity. A known undated debt may receive a conservative reserve but is never described as realized failed cash.

`through_day_90_sensitivity_only` covers offsets 0 through 90 (91 dates) exclusively for diagnostic comparison. `buildDiagnosticForecast` and `calculateDiagnosticCapacity` are explicit diagnostic APIs; ordinary `buildForecast` has no horizon selector, and production `calculateCapacity` rejects diagnostic or mismatched horizons with a blocking issue. Production commands do not accept a horizon flag. The separate sensitivity command runs both interpretations for every active request and reports category/date/capacity changes, a deterministic affected-ID sample, and exact deltas by currency. Signed delta is diagnostic 91-date capacity minus production 90-date capacity. Monetary metrics exclude null results and include zero changes; changed-only medians are separately labeled. Medians use exact addition and multiplication by 0.5, never binary floating point. No sample outputs influence this policy choice.
...
Scenario safety is checked at opening, after each ordered movement, and at closing. Diagnostic purchase debits occur after opening reserves **before any same-day cash**, in either intraday hypothesis. Stable IDs only break ties within a financial phase. Capacity takes the least headroom across valid scenarios and verifies that exact boundary through injected simulation. It is uncapped by requested price and receives no preference/deadline/spending-change filtering. It is never rounded upward.

Result dimensions are independent and machine-readable; they are not affordability categories. Baseline `status` is `valid`, `baseline_unsafe`, `blocked`, or `conservative_unresolved`. Blocking input takes precedence, then liability uncertainty, then an existing baseline breach; `baselineBreached` independently preserves any observed breach. Only `valid` exposes an exact capacity. `incrementalCapacity` distinguishes `positive_incremental_capacity`, genuine `zero_incremental_capacity`, and `not_calculable` with its baseline reason and null amount. `fullPaymentFeasibility` distinguishes `full_payment_supported` with a date, `no_full_payment_within_horizon` on a valid baseline, and `not_calculable` with `baseline_unsafe`, `blocked`, or `conservative_unresolved` reason. Thus zero capacity can coexist with a valid baseline and a supported later full-payment date, or with no full-payment date; baseline breach is distinct from an excessive requested price. Compatibility fields `maximumImmediatePayment` and `earliestFullPaymentDate` are derived from those same results and never substitute zero for missing data. Every null earliest date has a structured reason. The sensitivity comparison's combined category uses baseline status when non-valid, otherwise full-payment absence or valid; it also reports independent baseline counts.
